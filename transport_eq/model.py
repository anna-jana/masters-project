...
def evolve(model, state, options):
    ...
    sol = solve_ivp(rhs, (start, end), state.initial,
             ...
             t_eval=None if options.num_steps is None else steps, # jac=transport_eq_jac,
             )
...
def calc_T_end(calc_axion_mass, axion_decay_rate, axion_parameter, T_end, T_RH, options):
    ...
    #    raise ValueError("both calc_axion_mass and T_end are passed to start_solving")
        ...
        if T_end > T_RH:
            T_end = calc_next_T(calc_axion_mass, axion_parameter, T_RH, options.num_osc)
        return T_end

def start(model, T_RH, axion_initial, options, calc_axion_mass, T_end):
    T_end = calc_T_end(calc_axion_mass, model.axion_decay_rate, model.axion_parameter, T_end, T_RH, options)
...
def restart(res, calc_axion_mass, axion_parameter, options):
    new_T_start = res.T[-1]
    new_T_end = calc_next_T(calc_axion_mass, axion_parameter, new_T_start, options.num_osc_step)
...
def solve(model, T_RH, axion_initial, f_a, options=default_solver_options, calc_axion_mass=None, debug=False):
    eta_B, red_chem_pot_B_minus_L, T, (theta, theta_dot) = \
            solve_to_end(model, T_RH, axion_initial, options=options, calc_axion_mass=calc_axion_mass, debug=debug)
    if model.axion_decay_rate != 0:
        ...
        if debug:
            end_time = time.time()
            print("decay time:", end_time - start_time)
    return eta_B
