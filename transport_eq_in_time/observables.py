...
Status = enum.Enum("Status", "OK ASYM_CONVERGENCE_FAILURE RELIC_DENSITY_CONVERGENCE_FAILURE AXION_OSCILLATES_BEFORE_INFLATION ")
...
def compute_observables(H_inf, Gamma_inf, axion_parameter, f_a, axion_model, axion_init,
    ...
    if energy_scale > H_inf:
        return np.nan, np.nan, np.nan, Status.AXION_OSCILLATES_BEFORE_INFLATION 
    ...
    while True:
        ...
        # reheating process
        sol_rh, T_and_H_fn, T_and_H_and_T_dot_fn = decay_process.solve(tmax_inf_time, rho_R_init, 
                                                                       rho_inf_init, scale, Gamma_inf)
        ...
        if debug:
            print("change by timescale:", delta_, "vs", rtol_asym)
