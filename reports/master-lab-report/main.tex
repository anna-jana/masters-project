...
As in the classical theory the lagrangian is invariant under the U(1) transformation of the B and L symmetries but the integration measure of the path-integral is not. This way one could also derive eq. \eqref{eq:B_and_L_current_equations} via the so called ?????-method ???????.
...
It can be shown that this homotopy group is divided into homotopy classes labeled by the value of the integral in eq. \eqref{eq:chern_simons_number} that takes integers values the Chern-Simons number $N_\mathrm{CW}$ ????????.
As the integers are a discrete set, one can not continuously deform a gauge configuration into another with a different Chern-Simons number.
Back in Minkowski space-time we can define the Chern-Simons number of a gauge configuration at time $t$ as ?????
\begin{align}
    N_{\mathrm{CS}} := \int \mathrm{d}^3 x \, K^0
\end{align}
Using the temporal gauge ???? defined by $A^0 = 0$ where $K^i = 0$ we can
...
The path integral for the transition rate can be expanded around this instanton solution in euclidean space-time. An introduction to this technique can be found in ????. The tunneling process itself is also called an instanton ??????.
...
This changes drastically once we consider the system at finite temperature.
Here a change in Chern-Simons number is enhanced by thermal fluctuations (they are also called sphalerons).
Below the electro-weak phase-transition the height of the barrier still
suppresses changes of $N_{\mathrm{CS}}$ but above the vanishing vacuum-expectation value of the Higgs field implies at large rate.
A state of the art computation of the sphaleron rate \cite{Cline:2006ts_Baryogenesis} \cite[eq. ]{sphaleron_rate_Moore_2011} gave
\begin{align}
    \Gamma_{WS} = (1.06 \pm 0.08) \times 10^{− 6} T^4
\end{align}
...
equations of motion that sits on top of the potential barrier between two vacuua (technically the saddle point). In a sense it is the "middle" of the instanton configuration. The thermal
In this regime this process is in equilibrium. From considering the
equilibrium relations between chemical potetials for different charges one
...
\end{align}
where $N_H$ is the Higgs doublets ($N_H = 1$ in the SM).
...
will washout the asymmetry in $B$. Since B - L is a conserved quantity in the standard model, physics beyond the standard model is needed.
