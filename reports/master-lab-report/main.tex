...
We consider the effective axion Lagrangian ??????
\begin{align}
    ...
    \mathcal{L}_{\mathrm{axion-SM}} &=
        - \frac{C_{W\tilde{W}}}{32 \pi^2} \frac{\phi}{f_a} W_a^{\mu \nu}\tilde{W}^a_{\mu \nu}
        - \frac{C_{G\tilde{G}}}{32 \pi^2} \frac{\phi}{f_a} G_a^{\mu \nu}\tilde{G}^a_{\mu \nu}
...
the Klein Gordon equation on an FLRW background ??????? with the effective potential $V_\mathrm{eff}$ for the zero-mode i.e.
\begin{align}
    \label{eq:axion_klein_gordon}
    \ddot{\phi} + 3 H \dot{\phi} + \frac{\partial V(\phi; T)}{\partial \phi} = 0.
\end{align}
The effective potetial for the axion field is generated from instanton effects.
For QCD axion from SU(3) instanton in the stnadard model strong sector and hidden sector axions from some hidden stronly coupled sector. It is then given approximatly as ???????
\begin{align}
...
The integer value $N$ is the axion domain-wall number, which we set to $N = 1$ as in the ???-model ?????.
...
\begin{align}
    V_\mathrm{eff} = \frac{1}{2} m_a^2 \left( \frac{\phi}{f_a} \right)^2,
\end{align}
...
Then the axion field is homogeneous over one Hubble patch and is described by eq. \eqref{eq:axion_klein_gordon} ?????.
If the potential and kinetic energy are of the same order, any initial motion of the axion is damped by Hubble friction. Therefore one can assume the initial conditions $\phi = \phi_i$ and $\dot{\phi} = 0$ at the end of inflation ?????.
Once the Hubble parameter is succinctly low for the axion to overcome Hubble friction the axion rolls down its potential and starts to oscillate. This roll-down generates the time derivative
...
The temperature scale for our model is rougmmmhly given by the equilibration temperature of the Weinberg operator i.e.
\begin{align}
...
\end{align}
i.e. we assume that after the decay of an inflaton particle, the decay products immediatly follow a thermal distribution, which is not true exactly.
The temperature start at zero since no raidation exits. Then is quickly, within one dynamical time, rises to a maximal
value ??????
\begin{align}
    T_\mathrm{max} = 1
\end{align}
After that the energy density is still dominated by the inflaton field (matter domination) until
$\rho_\phi = \rho_R$, were the reheating temperature ?????????
\begin{align}
    T_\mathrm{RH} = 1
\end{align}
