...
\begin{abstract}
  ...
  We do this using a previously developed formalism based on a transport equation derived in linear response theory, which we discuss in detail. We apply this formalism to concrete model for the first time.
...
\end{align}
After the end of inflation the inflaton field will still dominate the energy density and the coherent oscillation have like hence the universe is matter dominated. This ends once the energy density of radiation is the same as the energy density of the inflaton field. 
The temperature at this time is called the reheating temperature.
...
So far we have derived the transport equation of a general set of charges $q_i$ and interactions $O_\alpha$. In this section we will specify these quantities for the standard model, which we will use later to compute the baryon asymmetry.
...
\end{table}
These charges can change by sphalerons and Yukawa couplings. As in ref. \cite{Domcke:2020kcp_Generic_Couplings}, we neglect flavor changing Yukawa interactions.
The processes are:
\begin{table}[H]
	\begin{center}
		\begin{tabular}{llll}
...
\todo{Furthermore we analyze the dependence of the asymmetry on the Hubble value at the end of inflation $H_\mathrm{inf}$ and the initial misalignment angle $\theta_i$.
TODO TODO}
...
\begin{table}[H]
	\begin{center}
		\begin{tabular}{ll}
			...
			Hubble parameter at the end of inflation & $H_\mathrm{inf}$ \\
			Inflaton decay rate  & $\Gamma_\mathrm{inf}$ \\
			Axion decay constant & $f$ where $f_\mathrm{eff} = ?????$ \\
...
\todo{fixed the curve intersection finding code (the dots are wrong in the plot)}
...
\todo{explain this weird thing with the two solutions}
