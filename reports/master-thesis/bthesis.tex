...
\todo{TODO: introduce all quantities????}
}:
\begin{equation}
	R_{\mu \nu} - \frac{1}{2} R g_{\mu \nu} = 8 \pi G T_{\mu \nu}.
\end{equation}
...
\end{align}
\todo{TODO: describe initial condition for reheating}
\begin{align}
...
The change between them is hence a quantum tunneling process called an instanton \footnote{\todo{TODO: explain formal defintion of instanton}}. See figure \ref{fig:sphaleron_cartoon} for an illustration.
Its rate is too low to have any significant effect, although this changes once we consider thermal processes i.e. so called sphalerons \footnote{\todo{TODO: explain sphaleron configurations}}.
...
\end{equation}
where $m_v$ is the left handed neutrino mass, $\nu_\mathrm{EW} = 174 \, \mathrm{GeC}$ is the Higgs vacuum expectation value,
$L$ the left-handed lepton doublet and $H$ the Higgs doublet.
The notation $C$ means charge conjuction and is given for some spinor $\psi$ as
\begin{equation}
	C \psi = i \gamma^2 \gamma^0 (\bar{\psi})^T
\end{equation}
and $\bar{\psi}$ is the Dirac adjoint spinor of the spinor $\psi$ given by
\begin{equation}
	\bar{\psi} = \psi^\dagger \gamma^0.
\end{equation}
The $\gamma^\mu$ are the gamma matrices.
The operator violates lepton number by $\Delta L = 2$.
...
\begin{equation}
	m_\nu = 0.05 \, \mathrm{eV}.
\end{equation}
The charge vector for the Weinberg operator for the combined first and second generation leptons and the third generation leptons are given as
\begin{align}
