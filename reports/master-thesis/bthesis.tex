...
For $\epsilon \lesssim |\theta| \lesssim \mathcal{O}(1)$ we find that
\begin{equation}
	\phi(\theta) = f \int_1^\theta \mathrm{d} \theta' \frac{1}{\theta'} = f \log(\theta')
\end{equation}
Finally we obtain the approximations for the potential for the canonical field $\phi$ in each of the three cases.

\subsection*{Slow-Roll}

\subsection*{Fast-Roll}
